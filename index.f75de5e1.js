...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    ths.forEach((th, clickedIndex)=>{
        th.addEventListener("click", ()=>{
            ...
            else nextDirection = "asc"; // Default to asc if none or desc
            ths.forEach((header, index)=>{
                if (index === clickedIndex) {
                    ...
                    header.querySelectorAll(".sort-indicator").forEach((ind)=>ind.remove()); // Видаляємо старі індикатори
                    ...
                    header.style.color = "";
                    header.querySelectorAll(".sort-indicator").forEach((ind)=>ind.remove());
                }
            });
            ...
            rowsArray.sort((rowA, rowB)=>{
                ...
                let compareResult;
                if (columnIndex === 0 || columnIndex === 1) compareResult = valueA.localeCompare(valueB);
                else {
                    const numA = parseFloat(valueA.replace(/[$,]/g, ""));
                    const numB = parseFloat(valueB.replace(/[$,]/g, ""));
                    if (isNaN(numA) && isNaN(numB)) compareResult = 0;
                    else if (isNaN(numA)) compareResult = 1; // Non-numbers go after numbers
                    else if (isNaN(numB)) compareResult = -1; // Numbers go before non-numbers
